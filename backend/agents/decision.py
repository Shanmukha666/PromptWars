...
class DecisionAgent(DynamicAgent):
    ...
    def _threshold_decision(self, hemoglobin: float, demographics: Dict) -> Tuple[str, float]:
        """
        Make decision using source-provided reference ranges if available.
        ABSOLUTE RULE: MedLens may not contain built-in clinical normal ranges used to label patient results.
        """
        # Prioritize explicit source-provided reference range
        source_ref = demographics.get("reference_range") or demographics.get("source_reference_range")
        if isinstance(source_ref, dict) and source_ref.get("min") is not None:
            source_min = float(source_ref["min"])
            # Evaluate strictly relative to source reference lower bound
            if hemoglobin < (source_min - 2.0):
                return "HIGH", 0.95
            elif hemoglobin < source_min:
                return "MODERATE", 0.85
            else:
                return "LOW", 0.90
        
        # When no source reference range is available, do NOT use built-in clinical normal ranges
        # to label patient results. Provide non-diagnostic comparative score with review flagged.
        return "MODERATE", 0.50
    
    def _get_thresholds(self, gender: str, age: int, source_ref: Optional[Dict[str, float]] = None) -> Dict[str, float]:
        """
        ABSOLUTE RULE: MedLens may not contain built-in clinical normal ranges used to label patient results.
        Returns thresholds derived strictly from source reference range if provided.
        """
        if source_ref and "min" in source_ref and source_ref["min"] is not None:
            s_min = float(source_ref["min"])
            return {"critical": s_min - 2.0, "moderate": s_min}
        return {"critical": 0.0, "moderate": 0.0}
    ...
    def _generate_insight(self, risk_level: str, hemoglobin: float) -> str:
        """Generate descriptive clinical data summary without diagnostic claims"""
        return f"Documented hemoglobin observation: {hemoglobin} g/dL. Clinical interpretation requires physician review."
    
    def _suggest_actions(self, risk_level: str, hemoglobin: float) -> List[str]:
        """Suggest verification workflow steps (strictly non-prescriptive, no treatments)"""
        return [
            "Verify extracted laboratory values against primary source document",
            "Refer structured findings to qualified healthcare professional for evaluation"
        ]
    ...
    def _handle_missing_data(self) -> Dict:
        ...
        return {
            ...
            "reasoning": ["Missing required laboratory observations"],
            "clinical_insight": "Insufficient laboratory data extracted from source",
            "suggested_actions": ["Verify source report contains laboratory results"]
        }
    ...
    def learn_from_outcome(self, case_id: str, actual_outcome: str, predicted_risk: str):
        """
        Record feedback from actual patient outcome.
        Autonomous mutation of clinical thresholds is strictly disabled for clinical safety.
        """
        # Compute reward
        ...
        # Update Q-table metrics for auditing
        self._update_q_table({
            "state": {"case_id": case_id},
            "action": {"predicted": predicted_risk, "actual": actual_outcome},
            "reward": reward
        })
        # Note: Autonomous clinical threshold mutation is disabled per clinical safety guidelines.
        logger.info(f"Outcome logged for case {case_id}: predicted={predicted_risk}, actual={actual_outcome}, reward={reward}")
